...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (action === 'get_print_data') {
            if (!pcb) return res.status(400).json({ status: 'error', message: 'Parameter PCB diperlukan' });
            const result = await client.query(`
                ...
                GROUP BY pt.picklist_number, pt.container_number, pt.huid, pt.container_type,
                         pt.weight_kg, an.no_sj, an.nama_toko, an.alamat, an.address
        ...
        switch (target) {
            ...
            case 'picking_transactions':
                queryText = `
                    ...
                    LEFT JOIN (
                        SELECT DISTINCT ON (picklist_number) picklist_number, nama_customer
                        FROM picklist_raw ORDER BY picklist_number, id
                    ) pr ON pt.picklist_number = pr.picklist_number
                    LEFT JOIN autoneon an ON pt.picklist_number = an.no_picking
                    ORDER BY pt.scanned_at DESC
                    LIMIT 500
                `;
                break;
            ...
            case 'outbound_explorer':
                queryText = `
                    SELECT
                        ...
                        pr.status,
                        COALESCE(an.nama_toko, pr.nama_customer, '-') AS nama_customer
            ...
            default:
                return res.status(400).json({ status: 'error', message: 'Target tidak valid' });
        }
