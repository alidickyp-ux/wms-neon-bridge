...
if (action === 'upload_snap' && req.method === 'POST') {
  ...
  // --- ACTION: LOGIN (DI DALAM HANDLER) ---
if (action === 'login' && req.method === 'POST') {
  const { username, password } = req.body;
  try {
    // Sesuaikan dengan nama tabel 'operators' dan kolom 'username'
    const sql = 'SELECT id, username, full_name, role FROM operators WHERE username = $1 AND password = $2';
