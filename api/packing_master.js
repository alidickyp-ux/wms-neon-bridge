...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            if (action === 'get_list') {
                const result = await client.query(`
                    ...
                    FROM picklist_raw p
                    WHERE LOWER(p.status) IN ('fully picked', 'partial picked', 'partially packed')
            ...
            if (action === 'get_history_list') {
                const result = await client.query(`
                    SELECT pt.picklist_number,
                           ...
                           (SELECT SUM(weight_kg) FROM (
                                SELECT DISTINCT container_number, weight_kg FROM packing_transactions t2 
                                WHERE t2.picklist_number = pt.picklist_number
                           ) AS unique_weights)::float AS total_weight
            ...
            if (action === 'get_next_container') {
                const result = await client.query(`SELECT COUNT(DISTINCT container_number)::int AS total FROM packing_transactions WHERE picklist_number = $1`, [pcb]);
            ...
            // F. GET PRINT DATA (DENGAN BARCODE 1D & QR)
        ...
        if (req.method === 'POST') {
            ...
            if (action === 'save_item') {
                const check = await client.query(`SELECT huid FROM packing_transactions WHERE picklist_number = $1 AND container_number = $2 LIMIT 1`, [picklist_number, container_number]);
